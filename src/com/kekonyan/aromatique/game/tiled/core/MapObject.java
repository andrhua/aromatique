...
public class MapObject implements Cloneable {
	...
	public MapObject(int id, int x, int y, int width, int height, float angle) {
	...
	public void setImageSource(String source) {
		...
		if (imageSource.length() > 0) {
			image = BitmapFactory.decodeFile(imageSource);
		} else {
			image = null;
		}
	 ...
	 */
	public Bitmap getImage(double zoom) {
		if (image == null)
			return null;
		...
		if (scaledImage == null || scaledImage.getWidth() != zoomedWidth
			...
			scaledImage = Bitmap.createScaledBitmap(image, zoomedWidth,
					zoomedHeight, true);
		}
