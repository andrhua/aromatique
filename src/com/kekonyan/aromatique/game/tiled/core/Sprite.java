...
public class Sprite
{
    ...
    public Sprite(Bitmap image, int fpl, int border, int totalFrames) {
        Tile[] frames = null;
        this.fpl = fpl;
        borderWidth = border;
        ...
        frameSize.width = image.getWidth() / (fpl + borderWidth * fpl);
        frameSize.height = (int) (image.getHeight() / (Math.ceil(totalFrames / fpl) + Math.ceil(totalFrames / fpl) * borderWidth));
