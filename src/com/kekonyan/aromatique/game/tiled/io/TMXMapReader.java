...
package com.kekonyan.aromatique.game.tiled.io;

import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import com.kekonyan.aromatique.GameActivity;
import com.kekonyan.aromatique.game.tiled.core.*;
import com.kekonyan.aromatique.game.tiled.util.Base64;
import com.kekonyan.aromatique.game.tiled.util.BasicTileCutter;
...
public class TMXMapReader
{
    ...
    private static int getAttribute(Node node, String attribname, int def) {
        ...
        if (attr != null) {
            return MathUtils.ceil(Float.valueOf(attr));
        } else {
            return def;
        }
    }
    ...
    private Bitmap unmarshalImage(Node t, String baseDir) throws IOException
    {
        Bitmap img = null;
        ...
        if (source != null) {
            ...
            img = BitmapFactory.decodeFile(source);
        } else {
    ...
    private MapObject readMapObject(Node t) throws Exception {
        ...
        final float angle=getAttribute(t, "rotation", 0);

        MapObject obj = new MapObject(id, x, y, width, height, angle);
        if (name != null)
            obj.setName(name);
        if (type != null)
            obj.setType(type);
        ...
        throws Exception
    {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if ("image".equalsIgnoreCase(child.getNodeName())) {
                ...
                Bitmap img = unmarshalImage(child, baseDir);
                tile.setImage(img);
    ...
    public Map readMap(String filename) throws Exception {
        ...
        InputStream is = GameActivity.assetManager.open(filename);
